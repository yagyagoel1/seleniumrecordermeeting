...
async function openMeet(driver) {
  ...
  try {
    await driver.get('https://meet.google.com/xmz-gegs-zdf');
    ...
    const elements = await driver.findElements({ className: 'GKGgdd' });
    if (elements.length > 1) {
        await elements[1].click(); // Turn off the camera
    }
...
async function startScreenshare(driver) {
    ...
    const response = await driver.executeScript(`
        ...
        window.navigator.mediaDevices.getDisplayMedia({
          ...
          const recordedChunks = await startRecording(combinedStream, 30000);
